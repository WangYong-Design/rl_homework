...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray,deter = False) -> np.ndarray:
        ...
        with torch.no_grad():
            distrib = self.forward(ptu.from_numpy(obs),deter)

            # action = ptu.to_numpy(distrib.sample())
    ...
    def forward(self, observation: torch.FloatTensor,deter = False):
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        logp_action = distrib.log_prob(actions)
        ...
        if self.nn_baseline:
            ...
            q_values = ptu.from_numpy((q_values - np.mean(q_values))/(np.std(q_values) + 1e-8))
            loss_baseline = ((q_values - self.baseline(observations))**2).mean()
